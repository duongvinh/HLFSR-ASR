...
class HLFSR_ASR(nn.Module):
	def __init__(self, angRes_in, angRes_out, n_blocks, channels):
		...
		self.UpSample = nn.Sequential(
            nn.Conv2d(angRes_in*angRes_in*channels, channels * angRes_out * angRes_out, kernel_size=1, stride=1, padding=0, bias=False),
	...
	def forward(self, x):
		...
		x_out = MI2StackView(HFEM_5,self.angRes_in)

		#Upscaling
		x_out= self.UpSample(x_out)
		x_out = MacPI2SAI(x_out,self.angRes_out)
